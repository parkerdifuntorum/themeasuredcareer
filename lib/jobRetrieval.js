...
function normalizeCompensation(min, max) {
  const numbers = [Number(min), Number(max)].filter((value) => Number.isFinite(value) && value > 0);
...
function locationForQuery(preferences = {}) {
  ...
  if ((preferences.modalities || []).includes("Remote")) return "remote";
...
export async function retrieveJobs(preferences = {}, recommendedTitles = []) {
  ...
  if (process.env.JSEARCH_API_KEY) {
    retrievalPromises.push(
      fetchJSearchJobs(queryTitles, preferences, recommendedTitles)
        .then((items) => {
          if (items.length) sources.push("JSearch");
          jobs.push(...items);
        })
        .catch((error) => {
  ...
  if (process.env.ADZUNA_APP_ID && process.env.ADZUNA_APP_KEY) {
    retrievalPromises.push(
      fetchAdzunaJobs(queryTitles, preferences)
        .then((items) => {
          if (items.length) sources.push("Adzuna");
          jobs.push(...items);
        })
        .catch((error) => {
  ...
  if (process.env.USAJOBS_EMAIL && process.env.USAJOBS_API_KEY) {
    retrievalPromises.push(
      fetchUsaJobs(queryTitles, preferences)
        .then((items) => {
          if (items.length) sources.push("USAJobs");
          jobs.push(...items);
        })
        .catch((error) => {
...
async function fetchJSearchJobs(queryTitles, preferences, recommendedTitles) {
  ...
  if (!response.ok) {
    throw new Error(`JSearch responded ${response.status}`);
  }
  ...
  return items.map((item) => {
    const description = normalizeText(item.job_description || item.job_highlights?.Responsibilities?.join(" ") || "No description provided.");
    ...
    return {
      ...
      modality: item.job_is_remote ? "Remote" : inferModality(item.job_employment_type, description),
      industry: inferIndustry(title, description),
      location: normalizeText(item.job_city && item.job_state ? `${item.job_city}, ${item.job_state}` : item.job_country || (item.job_is_remote ? "Remote" : "Not listed")),
      skillMatch: 80,
      source: "JSearch",
...
async function fetchUsaJobs(queryTitles, preferences) {
  ...
  if (!response.ok) {
    throw new Error(`USAJobs responded ${response.status}`);
  }
  ...
  return items.map((wrapper) => {
    ...
    const locations = (item.PositionLocation || []).map((loc) => loc.LocationName).filter(Boolean).join(", ");

    return {
      title,
      company: normalizeText(item.OrganizationName || "Federal agency"),
      compensation: normalizeCompensation(item.PositionRemuneration?.[0]?.MinimumRange, item.PositionRemuneration?.[0]?.MaximumRange),
      modality: inferModality(title, description),
      industry: "Government",
