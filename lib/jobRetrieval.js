...
/**
 * Less aggressive Greenhouse filtering:
 * - Previously filtered out too many roles using exact-ish title words.
 * - Now we include most roles from configured boards and let OpenAI ranking decide.
 * - Only drop obvious internships if user did not search for internship.
 */
...
export async function retrieveJobs(preferences = {}, recommendedTitles = []) {
  ...
  // Primary source: Google Jobs via SerpAPI.
  ...
  return {
    jobs: dedupeJobs(jobs).slice(0, 150),
    sources,
    usedFallback: false,
    providerErrors,
  };
...
async function fetchSerpApiGoogleJobs(queryTitles, preferences) {
  const titleQueries = queryTitles.slice(0, 4);
  const allJobs = [];
  ...
  for (const searchTitle of titleQueries) {
    ...
    const query = `${searchTitle}${industryQueryText(preferences)}${remoteTerm} jobs`.trim();
    ...
    allJobs.push(
      ...items.map((item) => {
        ...
        const detectedExtensions = Array.isArray(item.detected_extensions)
          ? item.detected_extensions
          : [];

        const salaryText =
          item.detected_extensions?.salary ||
          item.extensions?.find?.((extension) => /\$|salary|year|hour/i.test(extension)) ||
          "";

        const isRemote =
          /remote/i.test(`${item.location || ""} ${item.description || ""} ${detectedExtensions.join(" ")}`);
        ...
        return {
          ...
          applyUrl: item.related_links?.[0]?.link || item.share_link || item.job_id || "https://www.google.com/search?q=" + encodeURIComponent(`${title} ${item.company_name || ""} jobs`),
...
async function fetchJSearchJobs(queryTitles, preferences, recommendedTitles) {
  ...
  for (const searchTitle of titleQueries) {
    const location = locationForQuery(preferences);
    const remoteTerm = wantsRemote(preferences) ? " remote" : "";
    const query = `${searchTitle}${industryQueryText(preferences)}${remoteTerm} ${location}${preferredSalaryQuery(preferences)}`.trim();
...
async function fetchAdzunaJobs(queryTitles, preferences) {
  ...
  for (const searchTitle of titleQueries) {
    ...
    url.searchParams.set("what", `${searchTitle}${industryQueryText(preferences)}`.trim());
    ...
    allJobs.push(
      ...items.map((item) => {
        ...
        return {
          ...
          skillMatch: 80,
          source: "Adzuna",
          description: description.replace(/<[^>]+>/g, "").slice(0, 700),
