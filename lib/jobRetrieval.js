...
function isTrustedDirectBoard(url) {
  const parsed = safeUrlObject(url);
  ...
  return trustedDirectBoards.some((domain) => hostEndsWith(host, domain));
}

function looksLikeDirectCareerUrl(url) {
  const parsed = safeUrlObject(url);
  if (!parsed) return false;
...
function chooseBestApplyUrl(candidates = [], company = "") {
  ...
  const scored = cleaned.map((url) => {
    ...
    if (isTrustedDirectBoard(url)) score += 45;
    if (looksLikeDirectCareerUrl(url)) score += 35;
