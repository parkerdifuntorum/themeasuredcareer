...
function titleLooksRelevant(jobTitle, queryTitles) {
  if (!queryTitles.length) return true;

  const title = jobTitle.toLowerCase();
  const queryWords = queryTitles
    .join(" ")
    .toLowerCase()
    .split(/\s+/)
...
export async function retrieveJobs(preferences = {}, recommendedTitles = []) {
  ...
  return {
    jobs: dedupeJobs(jobs).slice(0, 100),
    sources,
    usedFallback: false,
    providerErrors,
  };
...
async function fetchGreenhouseJobs(queryTitles, preferences) {
  ...
  for (const board of boards) {
    ...
    const normalizedJobs = items
      .filter((item) => titleLooksRelevant(item.title || "", queryTitles))
      .map((item) => {
        const title = normalizeText(item.title || "Untitled role");
