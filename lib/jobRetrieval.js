...
function validApplyUrl(url) {
  return typeof url === "string" && url.startsWith("http") && !url.includes("themeasuredcareer.com");
}

function cleanApplyUrl(url) {
  return validApplyUrl(url) ? url : null;
}
    ...
    allJobs.push(
      ...items.map((item) => {
        ...
        return {
          ...
          applyUrl:
            cleanApplyUrl(item.related_links?.find?.((link) => validApplyUrl(link?.link))?.link || item.share_link),
    ...
    allJobs.push(
      ...items.map((item) => {
        ...
        return {
          ...
          source: "JSearch",
          description: description.slice(0, 900),
          applyUrl: cleanApplyUrl(item.job_apply_link || item.job_google_link),
