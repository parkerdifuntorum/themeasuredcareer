...
export function scoreApplyUrl(url, job = {}) {
  ...
  if (isTrustedAtsUrl(url)) score += 50;
  ...
  if (/indeed|linkedin|ziprecruiter|glassdoor|monster|talent\.com|simplyhired/i.test(host)) {
    score -= 25;
  }
...
export async function enrichSingleJobApplyUrl(job) {
  ...
  return {
    ...job,
    applyUrl: bestExisting || null,
    applyUrlSource: bestExisting ? "provider-low-confidence" : "not-found",
    applyUrlConfidence: bestExisting ? scoreApplyUrl(bestExisting, job) : 0,
  };
}
