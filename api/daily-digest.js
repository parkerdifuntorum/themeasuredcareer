...
function rankJobsForSubscription(subscription) {
...
function renderJobRows(jobs = [], email = "") {
  return jobs
    .map((job, index) => {
...
async function sendDailyDigest(subscription) {
  const jobs = await rankJobsForSubscription(subscription)();
        ...
        <div style="max-width:760px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e5e7eb;">
          ...
          <div style="padding:22px;">
            <table style="width:100%;border-collapse:collapse;">
              ${renderJobRows(jobs, subscription.email)}
            </table>
            <p style="font-size:12px;color:#667085;margin-top:22px;">
              You are receiving this because you subscribed to daily job updates.
...
export default async function handler(req, res) {
  ...
  if (!process.env.OPENAI_API_KEY || !process.env.RESEND_API_KEY) {
    return res.status(500).json({
      error: "OPENAI_API_KEY and RESEND_API_KEY are required.",
    });
  }
